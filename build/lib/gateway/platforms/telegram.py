...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_callback_query(
        ...
        # --- Task confirm callbacks (tk:task_name) ---
        if data.startswith("tk:"):
            task_name = data[3:]  # Remove "tk:" prefix
            ...
            if not self._is_callback_user_authorized(
                ...
                await query.answer(text="⛔ 你没有权限确认此任务。")
                return

            await query.answer(text="✅ 已确认")

            # Edit message to show confirmation, remove button
            try:
                from gateway.platforms.telegram import _escape_mdv2
                confirm_text = f"✅ {_escape_mdv2(task_name)}"
                await query.edit_message_text(
                    text=confirm_text,
                    parse_mode=ParseMode.MARKDOWN_V2,
                    reply_markup=None,
            ...
            # Update pending_tasks.json: mark matching task as completed
            try:
                from hermes_constants import get_hermes_home
                import json
                home = get_hermes_home()
                tasks_path = home / "data" / "pending_tasks.json"
                if tasks_path.exists():
                    data = json.loads(tasks_path.read_text(encoding="utf-8"))
                    updated = False
                    for t in data.get("tasks", []):
                        if t.get("name") == task_name and t.get("status") == "pending":
                            t["status"] = "completed"
                            updated = True
                    if updated:
                        ...
                        logger.info(
                            "Task confirm callback: marked '%s' as completed (user=%s)",
                            task_name, caller_id,
                        )
            except Exception as exc:
                logger.warning("Task confirm callback: failed to update tasks file: %s", exc)
            return
