...
class SessionDB:
    ...
    def _insert_session_row(
        ...
        def _do(conn):
            conn.execute(
                """INSERT OR IGNORE INTO sessions (id, source, user_id, chat_id, model, model_config,
    ...
    def get_messages_for_chat(
        ...
        """Load messages across all sessions for a given chat_id.

        Returns user/assistant messages ordered by timestamp descending,
    ...
    def search_messages_chat(
        ...
        """Full-text search across messages for a specific chat_id."""
        ...
        for row in rows:
            r = dict(row)
            content = self._decode_content(r.get("content"))
